...
describe('Aviator Test', () => {
    ...
    describe('Login Test', () => {
        ...
        test('Login reset password invalid account', async () => {
            ...
            await expect(welcomeTxt[0]).toBe('Unable to find your account.')
        });
        ...
        test('Login reset password', async () => {
            ...
            var welcomeTxt = await page.$$eval(selector.flash_notice,
                elements=> elements.map(item=>item.textContent));       
            await expect(welcomeTxt[0]).toBe('You will receive an email from us shortly to reset your password.')
        });
