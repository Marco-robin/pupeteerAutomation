...
describe('Aviator Test', () => {
  ...
  test('Rewards', async()=>{
      ...
      await page.waitForSelector(selector.rewards);
      await page.click(selector.rewards);
      /*await page.waitForSelector(selector.award);
     // await page.click(selector.award);
      await page.select(selector.award, data.award);
      await page.type(selector.award_dollars,data.price_in_dollars);
      await page.waitForSelector(selector.flash_notice);
      var welcomeTxt = await page.$$eval(selector.flash_notice,
        elements=> elements.map(item=>item.textContent));       
